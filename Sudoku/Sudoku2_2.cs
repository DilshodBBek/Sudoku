    ...
    class Sudoku2
    {
        ...
        public bool LineMinus()
        {
            ...
            for (int y = 0; y < BaseNumder; y++)
                ...
                {
                    if (!arr[x, y])
                    {                //Если клетка не определена
                        for (int LineNumder = 0; LineNumder < BaseNumder; LineNumder++)
                        {
                            if (arr[x, y] - arr[x, LineNumder])
                            {    //Если после вычитания клетка определена
                                n++;
                                break;
                            }
        ...
        public bool LineArr()
        {
            ...
            for (int Line = 0; Line < BaseNumder; Line++)
                for (int CellArr = 0; CellArr < BaseNumder; CellArr++)
                { //Здесь - проход по массиву вариантов клетки
